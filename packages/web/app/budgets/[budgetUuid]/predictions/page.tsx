...
export default function PredictionsPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setIsLoading(true);
                // Fetch categories using the client-side function
                const categoriesData = await getCategories(budgetId);
                setCategories(categoriesData);

                // Forecast data is fetched directly in the PredictionChart component
                // since it already uses the getPrediction function
                setForecast({} as MonthlyForcast); // Placeholder since we don't need the forecast directly
            } catch (err) {
    ...
    return (
        <div className="container mx-auto p-4 space-y-8">
            ...
            <div className="h-[600px] w-full bg-base-200 rounded-lg p-4">
                {forecast && categories && (
                    <PredictionChart
                        forecast={forecast}
